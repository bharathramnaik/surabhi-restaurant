...
export default function EmployeesPage() {
  ...
  const verifyPin = () => { if (pinInput === (settings.adminPin ?? "1234")) { setIsAdmin(true); setPinDialogOpen(false); setPinInput(""); toast.success("Admin access granted"); } else toast.error("Invalid PIN"); };
  ...
  const handleSaveSalary = () => {
    if (!salaryForm.employeeId) { toast.error("Select employee"); return; }
      ...
      <div className="p-4 md:p-6 pb-20 md:pb-6 space-y-4">
        ...
        <Card className="max-w-sm mx-auto mt-8">
          <CardContent className="pt-6 pb-6 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">Enter admin PIN to view and manage employees</p>
            <Input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="PIN" maxLength={6} onKeyDown={(e) => { if (e.key === "Enter") verifyPin(); }} className="max-w-40 mx-auto text-center text-lg" />
    ...
    <div className="p-4 md:p-6 pb-20 md:pb-6 space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {activeTab === "salary" && <Button size="sm" onClick={() => openAddSalary()} className="cursor-pointer"><IndianRupee className="w-4 h-4 mr-1" /> Add Salary</Button>}
        </div>
      </div>
      <div className="flex gap-2">
        <Button variant={activeTab === "employees" ? "default" : "secondary"} size="sm" onClick={() => setActiveTab("employees")} className="cursor-pointer"><Users className="w-4 h-4 mr-1" /> Employees</Button>
        <Button variant={activeTab === "salary" ? "default" : "secondary"} size="sm" onClick={() => setActiveTab("salary")} className="cursor-pointer"><IndianRupee className="w-4 h-4 mr-1" /> Salary</Button>
      </div>
      {activeTab === "employees" && (
        <>
          <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search employees..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" /></div>
                ...
                <Card key={emp.id} className="shadow-sm"><CardContent className="pt-3 pb-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2"><p className="font-semibold text-sm">{emp.name}</p><span className={cn("text-[10px] px-1.5 py-0.5 rounded-full", emp.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500")}>{emp.active ? "Active" : "Inactive"}</span></div>
            ...
            return (
              <Card key={emp.id} className="shadow-sm"><CardContent className="pt-3 pb-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button size="sm" variant="secondary" onClick={() => openAddSalary(emp.id)} className="cursor-pointer text-xs"><Plus className="w-3 h-3 mr-1" /> Record</Button>
                </div>
                {empRecords.length === 0 ? <p className="text-xs text-muted-foreground">No salary records</p> : (
                  <div className="space-y-1">{empRecords.slice(0, 6).map((r) => (
                    <div key={r.id} className="flex items-center justify-between text-xs py-1 border-b border-border last:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={cn("px-1.5 py-0.5 rounded-full text-[10px]", r.paid ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>{r.paid ? "Paid" : "Unpaid"}</span>
                        {!r.paid && <Button size="sm" variant="ghost" className="cursor-pointer text-[10px] h-5" onClick={() => updateSalaryRecord(r.id, { paid: true })}>Mark Paid</Button>}
                      </div>
                    </div>
                  ))}</div>
                )}
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>Phone</Label><Input value={empForm.phone} onChange={(e) => setEmpForm({ ...empForm, phone: e.target.value })} /></div>
            </div>
            <div className="space-y-1"><Label>Email</Label><Input type="email" value={empForm.email} onChange={(e) => setEmpForm({ ...empForm, email: e.target.value })} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><Label>Role</Label><Select value={empForm.role} onValueChange={(v) => setEmpForm({ ...empForm, role: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{ROLES.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-1"><Label>Shift</Label><Select value={empForm.shift} onValueChange={(v) => setEmpForm({ ...empForm, shift: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SHIFTS.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>Monthly Salary (₹)</Label><Input type="number" min={0} value={empForm.salary} onChange={(e) => setEmpForm({ ...empForm, salary: Number(e.target.value) })} /></div>
            </div>
            <div className="flex items-center gap-2"><Switch checked={empForm.active} onCheckedChange={(v) => setEmpForm({ ...empForm, active: v })} id="active" /><Label htmlFor="active">Active Employee</Label></div>
            <div className="space-y-1"><Label>Notes</Label><Textarea rows={2} value={empForm.notes} onChange={(e) => setEmpForm({ ...empForm, notes: e.target.value })} /></div>
      ...
      <Dialog open={salaryDialogOpen} onOpenChange={setSalaryDialogOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><Label>Month</Label><Select value={String(salaryForm.month)} onValueChange={(v) => setSalaryForm({ ...salaryForm, month: Number(v) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{MONTHS.map((m, i) => <SelectItem key={i + 1} value={String(i + 1)}>{m}</SelectItem>)}</SelectContent></Select></div>
              <div className="space-y-1"><Label>Year</Label><Input type="number" min={2020} value={salaryForm.year} onChange={(e) => setSalaryForm({ ...salaryForm, year: Number(e.target.value) })} /></div>
            </div>
            <div className="space-y-1"><Label>Amount (₹)</Label><Input type="number" min={0} value={salaryForm.amount} onChange={(e) => setSalaryForm({ ...salaryForm, amount: Number(e.target.value) })} /></div>
            <div className="flex items-center gap-2"><Switch checked={salaryForm.paid} onCheckedChange={(v) => setSalaryForm({ ...salaryForm, paid: v })} id="paid" /><Label htmlFor="paid">Already Paid</Label></div>
            <div className="space-y-1"><Label>Notes</Label><Input value={salaryForm.notes} onChange={(e) => setSalaryForm({ ...salaryForm, notes: e.target.value })} /></div>
            <div className="flex gap-2 justify-end pt-2">
