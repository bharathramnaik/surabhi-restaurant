...
type ItemForm = { name: string; nameKn: string; description: string; descriptionKn: string; price: number; categoryId: string; available: boolean; isVeg: boolean; };
const emptyForm = (): ItemForm => ({ name: "", nameKn: "", description: "", descriptionKn: "", price: 0, categoryId: "", available: true, isVeg: true });

export default function MenuPage() {
  const { t, i18n } = useTranslation("common");
  const isKn = i18n.language === "kn";
  const categories = useQuery(api.menuCategories.getAll) ?? [];
  const items = useQuery(api.menuItems.getAll) ?? [];
  const addItem = useMutation(api.menuItems.add);
  const updateItem = useMutation(api.menuItems.update);
  const deleteItem = useMutation(api.menuItems.remove);
  const addCat = useMutation(api.menuCategories.add);
  const deleteCat = useMutation(api.menuCategories.remove);
  ...
  const openEdit = (item: typeof items[0]) => {
    setForm({ name: item.name, nameKn: item.nameKn, description: item.description, descriptionKn: item.descriptionKn, price: item.price, categoryId: item.categoryId, available: item.available, isVeg: item.isVeg });
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) await updateItem({ id: editingId, ...form });
  ...
  const handleDelete = async (id: Id<"menuItems">) => {
    ...
    await deleteItem({ id }); toast.success(t("msg.deleted"));
  };
  const handleAddCat = async () => {
    if (!newCatName) return;
    await addCat({ name: newCatName, nameKn: newCatNameKn || newCatName, sortOrder: categories.length + 1 });
    setNewCatName(""); setNewCatNameKn(""); toast.success(t("msg.saved")); setCatDialogOpen(false);
  };
  if (categories.length === 0 && items.length === 0) return <div className="p-6"><Skeleton className="h-48 w-full" /></div>;
  return (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={selectedCategory} onValueChange={setSelectedCategory}>
          ...
          <SelectContent>
            ...
            {categories.map((c) => <SelectItem key={c._id} value={c._id}>{isKn ? c.nameKn : c.name}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      {categories.filter((cat) => selectedCategory === "all" || cat._id === selectedCategory).map((cat) => {
        ...
        return (
          <div key={cat._id}>
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="sm" className="text-destructive h-6 text-xs cursor-pointer" onClick={() => { if (confirm(t("msg.confirm_delete"))) { void deleteCat({ id: cat._id }); } }}><Trash2 className="w-3 h-3" /></Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {catItems.map((item) => (
                <Card key={item._id} className={cn("shadow-sm", !item.available && "opacity-60")}>
                  <CardContent className="pt-4 pb-4">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1 ml-2">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive cursor-pointer" onClick={() => { void handleDelete(item._id); }}><Trash2 className="w-3.5 h-3.5" /></Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>{t("label.category")} *</Label><Select value={form.categoryId} onValueChange={(v) => setForm({ ...form, categoryId: v })}><SelectTrigger><SelectValue placeholder="Select..." /></SelectTrigger><SelectContent>{categories.map((c) => <SelectItem key={c._id} value={c._id}>{isKn ? c.nameKn : c.name}</SelectItem>)}</SelectContent></Select></div>
            </div>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <Button onClick={() => { void handleSave(); }} className="cursor-pointer">{t("btn.save")}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-sm"><DialogHeader><DialogTitle>Add Category</DialogTitle></DialogHeader>
          <div className="space-y-3">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={() => { void handleAddCat(); }} className="cursor-pointer">{t("btn.save")}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
