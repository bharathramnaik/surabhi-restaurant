...
export default function InventoryPage() {
  ...
  const handleSave = () => {
    if (!form.name) { toast.error(t("msg.name_required")); return; }
  ...
  return (
    <div className="p-4 md:p-6 pb-20 md:pb-6 space-y-4">
      ...
      {lowStockItems.length > 0 && (
          ...
          <div className="flex-1 min-w-0"><p className="text-sm font-medium text-amber-800">{t("label.low_stock_alert")}</p><p className="text-xs text-amber-600 truncate">{lowStockItems.map((i) => `${i.name} (${i.quantity} ${i.unit})`).join(", ")}</p></div>
        </div>
      )}
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={filterCategory} onValueChange={setFilterCategory}>
          ...
          <SelectContent><SelectItem value="all">{t("label.all_categories")}</SelectItem>{CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
        </Select>
      </div>
            ...
            <CardContent className="pt-3 pb-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                 ...
                 <div className="text-right"><p className={cn("text-sm font-bold", item.quantity <= item.minStock ? "text-amber-600" : "")}>{item.quantity} {item.unit}</p><p className="text-[10px] text-muted-foreground">{t("label.min_stock")}: {item.minStock} {item.unit}</p></div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{editingId ? t("btn.edit") : t("btn.add")} {t("nav.inventory").slice(0, -1)}</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1"><Label>{t("label.name")} *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Tomatoes" /></div>
            <div className="space-y-1"><Label>{t("label.category")}</Label><Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><Label>{t("label.quantity")}</Label><Input type="number" min={0} value={form.quantity} onChange={(e) => setForm({ ...form, quantity: Number(e.target.value) })} /></div>
              <div className="space-y-1"><Label>{t("label.unit")}</Label><Select value={form.unit} onValueChange={(v) => setForm({ ...form, unit: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="kg">kg</SelectItem><SelectItem value="L">L</SelectItem><SelectItem value="pcs">pcs</SelectItem><SelectItem value="g">g</SelectItem></SelectContent></Select></div>
            </div>
            <div className="space-y-1"><Label>{t("label.min_stock")}</Label><Input type="number" min={0} value={form.minStock} onChange={(e) => setForm({ ...form, minStock: Number(e.target.value) })} /></div>
